...
test_cnpg_cluster_imagename_digest_pinned if {
	count(deny) == 0 with input as {"apiVersion": "postgresql.cnpg.io/v1", "kind": "Cluster", "spec": {"imageName": "registry.example.invalid/fixture/postgres:18.3-fixture@sha256:1111111111111111111111111111111111111111111111111111111111111111"}}
}

test_cnpg_cluster_imagename_floating_tag_rejected if {
	count(deny) > 0 with input as {"apiVersion": "postgresql.cnpg.io/v1", "kind": "Cluster", "spec": {"imageName": "registry.example.invalid/fixture/postgres:18.3-fixture"}}
}

test_cnpg_cluster_imagename_no_tag_rejected if {
	count(deny) > 0 with input as {"apiVersion": "postgresql.cnpg.io/v1", "kind": "Cluster", "spec": {"imageName": "registry.example.invalid/fixture/postgres"}}
}
