...
test_valid_declared_workload_gsm_renders_pass if {
	count(deny) == 0 with input as valid_rendered_secrets_input with data.render_contract as valid_discovery with data.rendered.workload_gsm_contracts as fixture_workload_gsm_contracts
}
...
test_sentinel_remote_ref_key_fails if {
	...
	msg in deny with input as bad with data.render_contract as valid_discovery with data.rendered.workload_gsm_contracts as fixture_workload_gsm_contracts
}
...
test_missing_required_discovered_overlay_fails if {
	...
	msg in deny with input as valid_rendered_secrets_input with data.render_contract as missing_main_discovery with data.rendered.workload_gsm_contracts as fixture_workload_gsm_contracts
}
...
test_second_declared_workload_contract_passes if {
	...
	count(deny) == 0 with input as multi_input with data.render_contract as multi_discovery with data.rendered.workload_gsm_contracts as multi_contracts
}
