...
3. Do not retry, edit, or second-guess Codex. Report the exit code it gave and the two file paths.

Return { exitCode, stdoutFile (=$out), stderrFile (=$err) }.`
...
Return { pass (true only when both letter and spirit are met), reportPath (=$r) }.`
...
Return { taskFile (=$dir/task.md), dir (=$dir) }.`
...
Then decide:
- If the draft is solid, change nothing and report noEditsApplied=true.
- If it is deficient (not self-contained, drops or distorts an original requirement, or mis-splits must-hold vs to-do), fix it. ${edit}
  ...
  Report noEditsApplied=false.

Return { noEditsApplied, createdFiles (any temp files you made; [] if none) }.`
}
